...
require_once(__DIR__.'/../Helpers/Helper.php'); //to fix with auto 
...
class ThemeService
{
	...
	{
		$params = \Route::current()->parameters();
		if(isset($params['module'])) return  $params['module'];
	...
	public static function getView($parz = []){
		...
		$path=collect(explode('.',$tmp))->map(function($item) use($params) {
			$item= Str::snake($item);
			if(isset($params[$item])) return $params[$item];
			return $item;
		})->implode('.');
	...
	public static function view($view=null){
		...
		*/
		...
		if(\in_admin()){
			...
			$view_extend='extend::layouts.default.admin.'.$act;
		}else{
		...
		if (\View::exists($view)) {
			...
			echo '<h3> pub_theme ['.$pub_theme.']</h3>';
			echo '<h3> ['.self::viewNamespace($view).']</h3>';
			$viewHints = View::getFinder()->getHints();
			$test=$viewHints['pub_theme'][0].'\\home\\index.blade.php';
			ddd(file_exists($test));
		...
		*/
		...
		if(is_object($row) && $row_type==''){
			$row_type=collect(config('xra.model'))->search(get_class($row));
		}
		...
		*/
		...
		if($row_type!=''){
			$item_view_arr=[
				$view.'.item.'.$row_type,
				$view_default.'.item.'.$row_type,
				$view_extend.'.item.'.$row_type,
				$view_extend.'.item',
			];
			$item_view=collect($item_view_arr)->first(function ($value, $key) {
